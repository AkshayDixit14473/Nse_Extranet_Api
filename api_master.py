...
def commonlisting(path, seg):
   ...
   url = "https://www.connect2nse.com/extranet-api/common/content/1.0?segment="+seg+"&folderPath="+path
...
def memberlisting(path, seg):
   ...
   # Replace <TOKEN> with your actual token
   # Set the API endpoint URL
   url = "https://www.connect2nse.com/extranet-api/member/content/1.0?segment="+seg+"&folderPath="+path
   # Set the headers for the request, including the Authorization header with the token
   headers = {'Authorization': 'Bearer ' + TOKEN}
   # Make the GET request
   response = requests.get(url, headers=headers)
